...
from .forms import CustomRegisterForm, BiddingCreationForm, BiddingConfirmationForm
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None and username!="abhijeet":
            auth_login(request, user)
            return redirect('app:home')
        elif user is not None and username=="abhijeet":
            auth_login(request, user)
...
@permission_required("admin")
def createbiddings(request):
    if request.method == 'POST':
        # create a form instance and populate it with data from the request:
        form = BiddingCreationForm(request.POST)
        print("form",form)
        ...
        form = BiddingCreationForm()

    return render(request, 'createbiddings.html', {'form': form})
...
def confirmbiddings(request):
    ...
    context = total_nonconfirmed_bids
    #print(total_nonconfirmed_bids, placement_name, company_name)
    return render(request, 'confirmbiddings.html',{'context':context})

@permission_required("admin")
def confirmbid(request,id):

    getbid =PlacementBid.objects.values('bid__bid_status','placement__placement_title','bid', 'bid_id', 'confirmed', 'id', 'offer', 'placement', 'placement_id','user__username', 'user_id').filter(id=id)
    #placement_name=Placement.objects.values('placement_title')
    #company_name = Company.objects.values('company_name')
    print(getbid)
    return render(request, 'confirmbid.html', {'getbid':getbid})


@permission_required("admin")
def confirmed(request,id,):
    total_nonconfirmed_bids = PlacementBid.objects.values('placement__placement_title', 'bid', 'bid_id', 'confirmed',
                                                          'id', 'offer', 'placement', 'placement_id', 'user__username',
                                                          'user_id').filter(confirmed=False)
    bidconfirmed = PlacementBid.objects.values('placement__placement_title', 'bid', 'bid_id', 'confirmed',
                                               'id', 'offer', 'placement', 'placement_id', 'user__username',
                                               'user_id').filter(id=id)
    # placement_name = Placement.objects.values('placement_title')
    # company_name = Company.objects.values('company_name')
    print(total_nonconfirmed_bids)
    if request.method=='POST':
    ...
    if confirm=="on":
        ...
        PlacementBid.objects.filter(id=id).update(confirmed=True)
    else:
        print("NotConfirmed",confirm)
    #PlacementBid.objects.filter(id=id).update(confirmed='some value')
    print(bidconfirmed)
    context = total_nonconfirmed_bids
    # print(total_nonconfirmed_bids, placement_name, company_name)
    return render(request, 'confirmbiddings.html', {'context': context})
